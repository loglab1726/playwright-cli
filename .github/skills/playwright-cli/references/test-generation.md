...
> **CI note:** Section 3 (Heal) below describes a live-browser, open-ended,
> ask-the-user-on-ambiguity healing workflow intended for **interactive/local**
> use. For **CI-driven, automated generate+run sessions**, Section 3 is
> **superseded** by the bounded, classified protocol in
> `.github/copilot-instructions.md` ("Execution & Self-Healing Protocol") and
> `docs/pipeline-plan.md` Sections 4–5: a hard 3-attempt cap enforced by
> `scripts/bounded-run.js` (not a self-reported counter), and failures that
> look like real app bugs or stale manual-test expectations get logged to
> `unresolved-test-failures.csv` immediately rather than triggering a
> stop-and-ask. Sections 0–2 of this file (generation mechanics, plan,
> generate) remain valid and unchanged for both interactive and CI use — only
> the Heal section's *stopping/asking behavior* differs in CI.
