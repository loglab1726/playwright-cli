...
3. The wrapper's exit code is authoritative: `1` means attempts remain, `2` means hard-stopped — do not attempt a 4th run after exit code `2`, regardless of how confident you are in a fix. The cap is per spec FILE, not per test — a hard stop can be leftover history from a different test in the same file (`node scripts/check-healing-state.js` shows current counters). Report it plainly rather than assuming your fix attempt itself failed.
